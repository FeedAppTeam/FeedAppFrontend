  ...
  contentChange() {
    ...
    openGallery() {
        ...
        this.imagePicker.getPictures(options).then((results) => {
            ...
            this.file.readAsDataURL(path, filename).then((base64string) => {
                ...
                this.token.saveUser(this.profile);
            });
        });
    }
    ...
    takephoto(type: number) {
        ...
        this.camera.getPicture(options).then((results) => {
            this.profile.img = 'data:image/jpeg;base64,' + results;
            this.token.saveUser(this.profile);
        });
    }
    ...
    async openDialog() {
        ...
        modal.onDidDismiss().then((result) => {
            this.profile.img = (result.data !== undefined  && result.data !== null &&
                                result.data.avatar !== undefined && result.data.avatar !== null) ?
                                result.data.avatar : this.profile.img;
            this.token.saveUser(this.profile);
        });
